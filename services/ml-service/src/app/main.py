"""
ML Service - FastAPI inference service for California Housing price predictions.
"""
...
app = FastAPI(title="ML Service - Housing Price Prediction")
...
async def load_model():
    ...
    if model_gcs_path:
        try:
            ...
            logger.info(f"Model loaded successfully from GCS in {load_time:.3f}s")
            return
        except ImportError:
    ...
    if not os.path.exists(model_path):
        ...
        raise FileNotFoundError(
            ...
            "Please ensure model artifact is available."
        )
...
async def root():
    ...
    return {
        "service": "ml-service",
        "message": "ML Service - California Housing Price Prediction",
        "endpoints": {
            "predict": "/predict",
            "metrics": "/metrics",
            "health": "/health"
...
async def health():
    ...
    return {"status": "healthy", "service": "ml-service", "model_loaded": is_healthy}
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        if latency > 1.0:
            logger.warning(f"Slow prediction detected: {latency:.3f}s")
        elif latency > 0.5:
        ...
        logger.error(f"Prediction error: {str(e)} (latency: {latency:.3f}s)", exc_info=True)
